...
class platform_lookup:
    def __init__(self, 
        datafile='../platform/GPL571-17391_DATA.txt',
    ...
    def lookup(self,probe,returns='ENTREZ_GENE_ID'):
        ...
        '''
        import numpy as np
        if returns == 'ENTREZ_GENE_ID':
            probes = self.df['ID'].values
        elif returns == 'ID':
            probes = self.df['ENTREZ_GENE_ID'].values
        else:
            print('returns must be ID or ENTREZ_GENE_ID')
    ...
    def convert(self, probe_list, returns='ENTREZ_GENE_ID', rule='first', verbosity=1, ref_set = None):
        '''
        ...
        for j,p in enumerate(probe_list):
            try:
                ...
                out = out.split('///')
                if rule=='overlap_first':
                    if ref_set is not None:
                        in_ref_set = False
                        i=0
                        while not in_ref_set:
                            new_out = out[i].strip()
                            if str(new_out) in ref_set:
                                out = new_out
                                in_ref_set = True
                            i+=1
                    else:
