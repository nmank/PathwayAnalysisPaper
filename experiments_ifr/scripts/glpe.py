...
sys.path.append('/data4/mankovic/GSE73072/')
...
def generate_pathway_selection_pipeline(glpe: Transform, class_attr: str) -> Pipeline:
    ...
    #ifr feature selection
    ifr = FeatureSelect(process=IFR(
                            model = SSVMClassifier(C=1, solver=LPPrimalDualPy, use_cuda=False),
                            partitioner = logo,
                            repetition = 50,
    ...
    # create pipeline
    pipeline = Pipeline(processes=(std, glpe, std, ifr),
